...
class JokaQueues extends Model {
		...
		if ($this->isConnected() && !empty($aPayloadId) && !empty($aPayload) && !empty($aPackageName) 
			...
			try {
				if ($this->execDML($theSql,$theParams,$theParamTypes)) {
					$this->logPayload(JokaPackage::fromArray($theParams),'in');
				}
			} catch (PDOException $pdoe) {
	...
	public function getIncomingPayloads($aPackageName='') {
		$theResultSet = array();
		if ($this->isConnected()) try {
			...
			$theSql .= ' ORDER BY transmit_ts LIMIT 20';
			$rs = $this->query($theSql,$theParams,$theParamTypes);
			$theResultSet = $rs->fetchAll(PDO::FETCH_CLASS, __NAMESPACE__.'\JokaPackage');
			$rs->closeCursor();
	...
	public function getOutgoingPayloads() {
		$theResultSet = array();
		if ($this->isConnected()) try {
			$rs = null;
			$myFinally = FinallyCursor::forDbCursor($rs);
			...
			throw new DbException($pdoe, 'getOutgoingPayloads() failed.');
		}
		return $theResultSet;
	}
