...
if (!empty($v->results)) {
	...
	$w .= '<th class="col_log_id">Log ID</th>';
	//$w .= '<th>Payload ID</th>';
	$w .= '<th class="col_direction">Direction</th>';
	...
	foreach((array) $v->results as $theResultRow) {
		...
		$theBodyRow .= '<td id="field_log_id" class="col_log_id">'.$theResultRow['log_id'].'</td>';
		//$theBodyRow .= '<td>'.$theResultRow['payload_id'].'</td>';
		$theBodyRow .= '<td id="field_direction" class="col_direction">'.$theResultRow['direction'].'</td>';
