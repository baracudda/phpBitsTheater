...
class AuthBasicAccount extends BaseActor
{
	...
	/**
	 * Add a new account using the given name.
	 * @param string $aAcctName - the username for the account.
	 * @return int Returns the ID of the newly created account.
	 */
	...
	/**
	 * Render webpage used for registering a new user account.
	 * @param string $aTask - URL segment used to determine what view/action to render.
	 * @return string Return the URL to redirect to, if any.
	 */
...
//				$this->debugLog(__METHOD__.' mobile auth fail; logging out'); //DEBUG
	...
	public function ajajCreate()
	{
		...
		if ( !empty ( $generatedAccountId ) )
		{
			...
			$theRegistrationResult = $dbAuth->registerAccount( $newAccountData, $accountGroup );
			$this->logStuff('New account registered: ', $theRegistrationResult);
	...
	public function ajajGet( $aAccountID=null )
	{
		...
		try
		{
			...
			$dbAuthGroups = $this->getProp('AuthGroups');
			$theReturn->groups = $dbAuthGroups->getAcctGroups($aAccountID);
			$this->addMobileHardwareIdsForAutoLogin( $theReturn );
			$this->setApiResults($theReturn);
		}
		catch( Exception $x )
	...
	{
		if ( empty($aAccountInfo) || empty($aAccountInfo->account_id) ) return; //trivial
		$dbAuth = $this->getProp( 'Auth' ) ;
		try
		{
	...
	{
		if ( empty($aAccountInfo) ) return; //trivial
		if ( empty($aAccountInfo->account_id) && empty($aAccountInfo->auth_id) ) return; //trivial
		$dbAuth = $this->getProp( 'Auth' ) ;
		try
		{
