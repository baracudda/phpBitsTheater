...
/**
 * Endpoints when using AuthOrgsBase model for auth accounts.
 */
class AuthOrgsBase extends BaseActor
{
	 ...
	 * @return AuthModel - an instance of the model
	...
	public function ajajUpdateProfile()
	{
		...
		if ( $bCurrentPwMatch && $bAuthorized ) {
			...
			try {
				...
				if ( !empty($theNewEmail) && strcmp($theOldEmail, $theNewEmail) != 0) {
				...
				if ($theAcctId==$this->getDirector()->account_info->account_id) {
					...
					if ( $dbAuth->isAccountInSessionCache() ) {
						$dbAuth->saveAccountToSessionCache($theChangedAccountInfo);
					}
				}
...
//			$this->debugLog( 'Reentry authenticated for [' . $aAuthID . '].' ) ;
	...
	public function ajajCreate()
	{
		...
		$v = $this->getMyScene();
		$aName 		= Strings::trim($v->account_name);
		$aPassword 	= Strings::trim($v->account_password);
		$aEmail 	= Strings::trim($v->email);
		$aGroupId 	= (!empty($v->account_group_id)) ? $v->account_group_id : $v->account_group_ids ;
		$aRegCode 	= Strings::trim($v->account_registration_code);
		...
		if ( !empty($v->account_is_active) ) {
			$newAccountData['account_is_active'] = $v->account_is_active;
		}
		//AuthOrg info
		$theOrgList = (!empty($v->account_org_ids)) ? $v->account_org_ids : null;
		...
		try {
			...
			if ( !empty($registrationResult['auth_id']) ) {
				$theRowSet = $this->searchAuthAccountSet(['auth_id' => $registrationResult['auth_id']]);
				if ( !empty($theRowSet) ) {
	...
	public function ajajUpdate( $aID = null )
	{
		...
		$v = $this->getMyScene();
		$theFilter = null;
		$theAuthID = Strings::trim($this->getRequestData($aID, 'auth_id', false));
		if ( !empty($theAuthID) ) {
			$theFilter = array('auth_id' => $theAuthID);
		}
		else {
			$theAcctID = Strings::trim($this->getRequestData($aID, 'account_id', false));
	 ...
	 *   with either the auth_id, name, email, or account_id. If this
	...
	protected function getAuthAccount( $aAccountLookup=null )
	{
		...
		try {
			...
			if ( $theOrgID == AuthModel::ORG_ID_4_ROOT ) $theOrgID = null;
			$dbAuthGroups = $this->getAuthGroupsModel();
	...
	{
		return [
				AuthModel::TOKEN_PREFIX_ANTI_CSRF . '%',
				AuthModel::TOKEN_PREFIX_COOKIE . '%',
				AuthModel::TOKEN_PREFIX_LOCKOUT . '%',
		];
	}
	...
	protected function setActiveStatus( $aAccountID, $bActive )
	{
		...
		$dbAuth = $this->getProp( 'Auth' ) ;
		try {
			if ( is_numeric($aAccountID) ) {
				$theAuth = $dbAuth->getAuthByAccountId($aAccountID);
			} else {
				$theAuth = $dbAuth->getAuthByAuthId($aAccountID);
			}
			if ( !empty($theAuth) ) {
	...
	protected function getOrgs( $aID=null )
	{
		...
		$theFieldList = array_values(array_diff(AuthOrg::getDefinedFields(), [
