...
use BitsTheater\models\AccountPrefs ;
 ...
 */
class AuthOrgsBase extends BaseModel implements IFeatureVersioning
{
	 ...
	 *  </li>
	 *  <li value="6">
	 *    Removed mobile references and the table.
	 *  </li>
	 * </ol>
	 * @var integer
	 */
	...
	protected function getTableDefSql($aTABLEconst, $aTableNameToUse=null)
	{
		...
		$logger = $this->getLogger()->withInfo([
			...
			'table_const' => $aTABLEconst,
			'table_name' => $aTableNameToUse,
		]);
		//$logger->logToDebug("get SQL for {$aTABLEconst}");
		switch($aTABLEconst) {
		case static::TABLE_AuthAccounts:
			...
			case self::DB_TYPE_MYSQL: default:
				//$logger->logToDebug("got SQL for {$aTABLEconst}");
		...
		//$logger->logToDebug("no SQL for {$aTABLEconst}");
		$logger->setInfo('table_const', null);
		$logger->setInfo('table_name', null);
	}
		...
		switch ($this->dbType())
		{
			...
			default:
				if ( !$this->exists($this->tnAuthAccounts) ||
					...
					return 4;
				else if ( $this->exists("{$this->tbl_}auth_mobile") )
					return 5;
		}//switch
	...
	public function upgradeFeatureVersion($aFeatureMetaData, $aScene)
	{
		...
		switch (true) {
			...
			case ( $theSeq < 6 ):
			if ( $this->exists("{$this->tbl_}auth_mobile") ) {
				$theSql = "DROP TABLE {$this->tbl_}auth_mobile";
				$this->execDML($theSql);
				$this->logStuff("[v6] dropped {$this->tbl_}auth_mobile table.");
			}
			case ( $theSeq < 7 ):
			{
	...
	protected function checkNewAuthAccountInfo( SqlBuilder $aSql )
	{
		...
		$theEmail = Strings::trim($aSql->getParamValue('email'));
		$this->checkIsNotEmpty('email', $theEmail);
		$aSql->setParamValue('email', $theEmail);
		
		//ensure account_name is not blank
		$theAcctName = Strings::trim($aSql->getParamValue('account_name'));
		if ( empty($theAcctName) ) {
			$theAcctName = Strings::trim($aSql->getParamValue(static::KEY_userinfo));
		}
		...
		if ( empty($thePwHash) ) {
			...
			$thePwInput = Strings::trim($aSql->getParamValue(static::KEY_pwinput));
	...
	public function addAuthAccount($aDataObject)
	{
		...
		if ( strtolower(Strings::trim($theSql->getParamValue('verified_ts')))=='now' ) {
	 ...
	 * @param string|array|null $aAuthGroups - (optional) auth group membership(s).
...
//		$theSql->logSqlDebug(__METHOD__); //DEBUG
	...
	public function getAuthByEmail($aEmail) {
		$theSql = "SELECT * FROM {$this->tnAuthAccounts} WHERE email = :email";
...
//			->logSqlDebug( __METHOD__, ' [DEBUG] ' )
	...
	public function getAuthByAuthId($aAuthId) {
		$theSql = "SELECT * FROM {$this->tnAuthAccounts} WHERE auth_id=:id";
	 ...
	 * @return ?int Returns the $delta needed to add to time() to get the
	 * cookie expiration date; NULL = no end date, 0 means do not use cookies.
	 */
	public function getCookieDurationInDays( ?string $aDuration=null ): ?int
	{
	 ...
	 * @return ?int Returns the cookie expiration timestamp parameter.
	 */
	public function getCookieStaleTimestamp(): ?int {
	...
	{
		//contents moved to TicketSweeper costume / CRON job.
		//nothing to do here, anymore
	}

	/**
	 * Check a venue for ticket information (auth account).
	 * @param Scene $aScene - the Scene object associated with an Actor.
	 * @param IWillCall $aVenue - the mechanism to check.
	 * @return ?object Returns the authorized account or else NULL.
	 */
	...
	public function setAuthIsActive( $bActive, $aAuthID, $aAcctID=null )
	{
		...
		try {
			...
			$this->removeTokensFor($aAuthID, $aAcctID,
					AuthPasswordReset::TOKEN_PREFIX . '%'
			);
	 ...
	 * @param string $aTaskID - token mapped to a task by this id.
	 * @param string $aToken - the task token.
	 * @return array|false Returns the stored task amount or FALSE if done.
	 */
	public function getTaskMeta( $aTaskID, $aToken )
	{
		$theSql = SqlBuilder::withModel($this);
		$theSql->startWith('SELECT account_id, updated_ts')
			->add('FROM')->add($this->tnAuthTokens)
			->startWhereClause()
			->mustAddParam('auth_id', $aTaskID)
			->setParamPrefix(' AND ')
			...
			;
		try {
			$row = $theSql->query()->fetch(\PDO::FETCH_ASSOC);
			if ( $row !== false ) {
				return [
					'amount_remaining' => $row['account_id'],
